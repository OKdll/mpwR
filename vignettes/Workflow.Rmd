...
## Import your data
Importing the output files from each software can be easily with `prepare_mpwR`. Please put all output files in one folder and follow the guidelines for naming the files. No other files/subfolders are allowed. Details are provided in the vignette [Import](https://okdll.github.io/mpwR/articles/Import.html).
...
For inter-software comparisons functions of the package **flowTraceR** [LINK] are incorporated. Software outputs are standardized and easily comparable.
...
## Plot
As a visual summary a radarchart for all analyses can be accessed via `plot_radarchart`.
